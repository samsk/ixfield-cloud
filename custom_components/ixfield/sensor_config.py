    ...
    if sensor_name in SENSOR_OVERRIDES:
        ...
        if "settable" in override:
            ...
            if api_settable != override_settable:
                _LOGGER.info(f"Override applied to {sensor_name}: API settable={api_settable}, Override settable={override_settable}, Reason: {reason}")
    ...
    for key, value in override.items():
        if key != "reason":  # Skip the reason field
            ...
            if original_value != value:
                _LOGGER.info(f"Override applied to {sensor_name}.{key}: API={original_value}, Override={value}, Reason: {reason}")
