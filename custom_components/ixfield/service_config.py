...
def find_matching_sensor(
    control_name: str, available_sensors: List[Dict[str, Any]]
) -> Optional[str]:
    """
    ...
    sensor_names = [
        sensor.get("name", "") for sensor in available_sensors if sensor.get("name")
    ]
    ...
    for pattern in sensor_patterns:
        if "*" in pattern:
            ...
            for sensor_name in sensor_names:
                if (
                    len(pattern_parts) == 2
                    and sensor_name.startswith(pattern_parts[0])
                    and sensor_name.endswith(pattern_parts[1])
                ):
                    _LOGGER.debug(
                        f"Found pattern match for {control_name}: {sensor_name} matches {pattern}"
                    )
                    return sensor_name
...
def get_available_controls(
    ...
    for control_name, control_config in mappings.items():
        ...
        if matching_sensor:
            ...
            sensor_data = next(
                (s for s in available_sensors if s.get("name") == matching_sensor), None
            )
            if sensor_data:
...
def validate_control_value(control_name: str, value: Any) -> tuple[bool, str]:
    ...
    if control_type == "boolean":
        ...
        return (
            False,
            f"Boolean control requires boolean or string value, got {type(value)}",
        )
        ...
        try:
            ...
            return (
                False,
                f"Temperature value {float_val} is outside reasonable range (-50 to 100)",
            )
        except (ValueError, TypeError):
            return (
                False,
                f"Temperature control requires numeric value, got {type(value)}",
            )
